...
function init() {
  ...
  const startAngleRad = THREE.MathUtils.degToRad(
    180 - BACKGROUND_ARC_DEGREES / 2
  );
  ...
  backgroundCylinder = new THREE.Mesh(
    backgroundCylinderGeometry,
    backgroundCylinderMaterial
  );
  ...
  const angleStep =
    numActualBoxes > 1 ? totalArcAngleRad / (numActualBoxes - 1) : 0;
  ...
  arcFocusTarget =
    numActualBoxes > 0
      ? new THREE.Vector3(
          circleCenterPoint.x,
          boxHeight / 2 + LIFT_HEIGHT,
          circleCenterPoint.z + arcRadius
        )
      : new THREE.Vector3(0, boxHeight / 2 + LIFT_HEIGHT, 0);

  cameraViews.push({
    navLabel: "Домой",
    viewId: 0,
    name: "View 1: Top Down",
    type: "general",
  ...
  const cameraHeightBoxFocus = 1.6,
    cameraOffsetX = 1.5,
    cameraOffsetZFromFrontFace = 4.0;
  allBoxes.forEach((box, index) => {
    const boxPos = box.position;
    const targetCameraPosition = new THREE.Vector3(
      boxPos.x + cameraOffsetX,
      cameraHeightBoxFocus,
  ...
  const lastBoxFocusView =
    cameraViews[BOX_FOCUS_VIEWS_START_INDEX + numActualBoxes - 1];
  ...
  cameraViews.push({
    navLabel: "До вращения",
    viewId: cameraViews.length,
    name: `View 4: Shifted Look Box ${numActualBoxes}`,
    type: "final_look",
    boxIndex: numActualBoxes - 1,
    position: finalCamPos_s,
    lookAt: finalLookAt_s,
    fov: lastBoxFocusView.fov,
  });
  ...
  cameraViews.push({
    navLabel: null,
    viewId: cameraViews.length,
    name: "View 5.1 (Base for Rotation)",
    type: "box_rotation",
    box: lastBoxForRotation,
    targetRotationY: initialRotY,
    text: "Бокс готов к вращению.",
    cameraViewIndexToClone: FINAL_LOOK_VIEW_INDEX,
  });
  cameraViews.push({
    navLabel: null,
    viewId: cameraViews.length,
    name: "View 5.2 (Rotate +90 deg)",
    type: "box_rotation",
    box: lastBoxForRotation,
    targetRotationY: initialRotY + Math.PI / 2,
    text: "Бокс повернут на 90° по часовой.",
    cameraViewIndexToClone: FINAL_LOOK_VIEW_INDEX,
  });
  cameraViews.push({
    navLabel: null,
    viewId: cameraViews.length,
    name: "View 5.3 (Rotate +180 deg)",
    type: "box_rotation",
    box: lastBoxForRotation,
    targetRotationY: initialRotY + Math.PI,
    text: "Бокс повернут на 180° по часовой.",
    cameraViewIndexToClone: FINAL_LOOK_VIEW_INDEX,
  });
  cameraViews.push({
    navLabel: null,
    viewId: cameraViews.length,
    name: "View 5.4 (Rotate +360 deg)",
    type: "box_rotation",
    box: lastBoxForRotation,
    targetRotationY: initialRotY + 2 * Math.PI,
    text: "Бокс совершил полный оборот по часовой.",
    cameraViewIndexToClone: FINAL_LOOK_VIEW_INDEX,
  });
  ...
  if (homeButton) {
    homeButton.addEventListener("click", (e) => {
      e.preventDefault();
      const targetIndex = parseInt(homeButton.dataset.viewIndex);
  ...
  const finalViewIndex = cameraViews.findIndex(
    (view) => view.type === "final_fade"
  );
...
function setupHeaderNavigation() {
  ...
  const navPoints = [
    ...
    {
      label: "Вращение",
      targetViewNameOrIndex: "View 5.1 (Base for Rotation)",
    },
  ...
  if (cameraNeedsAnimation) {
    tl.to(
      camera.position,
      {
        ...
        duration: duration,
        ease: "power2.inOut",
        onUpdate: () => camera.lookAt(animatedLookAtTarget),
      },
      0
    );
    tl.to(
      animatedLookAtTarget,
      { ...actualLookAt, duration: duration, ease: "power2.inOut" },
      0
    );
    tl.to(
      camera,
...
function handleSceneAndFooterState(targetView, prevView) {
  ...
  const wasInRotationSequence =
    prevType === "box_rotation" ||
    prevType === "final_look" ||
    prevType === "final_fade";
  const isEnteringGeneralSequence =
    targetType !== "box_rotation" &&
    targetType !== "final_look" &&
    targetType !== "final_fade";

  if (wasInRotationSequence && isEnteringGeneralSequence) {
    rotatingBox.rotation.y = rotatingBox.userData.initialRotationY;
  }

  const isRotationOrFade =
    targetType === "box_rotation" || targetType === "final_fade";
  ...
  allBoxes.forEach((b) => {
    if (isRotationOrFade) {
      ...
      if (b === targetView.box) {
        b.material.opacity =
          targetType === "final_fade" && isAnimating ? b.material.opacity : 1;
      }
    } else {
      b.visible = true;
  ...
  if (targetType === "general") {
    ...
    textPanelUI.style.opacity =
      targetType === "final_fade" && isAnimating
        ? textPanelUI.style.opacity
        : 1;
  }
}
